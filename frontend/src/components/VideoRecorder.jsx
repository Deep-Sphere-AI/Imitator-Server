...
export default function VideoRecorder({ recording, setRecording, chat, setChat, loading, setLoading }) {
  ...
  return (
    <div className="w-1/2 p-6 flex flex-col items-center bg-gradient-to-b from-gray-900 to-gray-800">
      <h2 className="text-2xl font-bold mb-6 text-white">📹 Record Your Signs</h2>
      ...
      <div className="relative mb-6">
        <video
          ...
          autoPlay
          muted
          className="w-full h-80 bg-black border-4 border-gray-600 rounded-2xl shadow-2xl object-cover"
        />
      ...
      <div className="flex flex-col items-center space-y-4">
        {!recording ? (
          <button
            ...
            disabled={loading}
            className="bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white px-8 py-4 rounded-full disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 transform hover:scale-105 shadow-lg flex items-center space-x-2 text-lg font-semibold"
          >
        ...
        <div className="text-center text-gray-300">
          <p className="text-sm">Maximum recording time: <span className="font-bold">60 seconds</span></p>
          <p className="text-xs mt-1 opacity-75">Make sure your signs are clearly visible</p>
        </div>
      </div>
    </div>
  )
