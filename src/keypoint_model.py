...
class Keypoints_Model:
    ...
    def normalize_keypoints(self, keypoint, n_keypoints = 133):
        #Keypoints a Tensor
        if not isinstance(keypoint, torch.Tensor):
            keypoint = torch.as_tensor(keypoint)    

        # Clean noise 
        keypoint, _ = self.filter_unstable_keypoints_to_num(keypoint, n_keypoints)
    ...
    def get_keypoints(self, video_path):
        keypoints = self.process_keypoints(video_path)
        keypoints_normalized = self.normalize_keypoints(keypoints, 130)
        if DEBUG: np.save("keypoints.npy", keypoints_normalized)
